...
So I built **SpecLock** — an MCP server with 42 tools that enforces non-negotiable constraints across sessions.
...
- Smithery: smithery.ai/servers/sgroy10/speclock (42 tools, works with Claude Code, Cursor, Windsurf, Cline, Codex)
...
42 tools across 5 categories: Memory, Tracking, Protection, Git, Intelligence.
...
SpecLock is an MCP server with 42 tools that:
...
SpecLock is an MCP server with 42 tools.
...
SpecLock is an MCP server with 42 tools organized in 6 categories:
...
**SpecLock** is an MCP server (42 tools) that acts as a **persistent brain** for your Lovable projects:
...
That's it. 42 tools. Zero config. [2/5]
...
SpecLock is an MCP server with 42 tools (including constraint templates, violation reports, and pre-commit hooks) that connects to Lovable as a custom connector. It does three things the manual approach can't:
